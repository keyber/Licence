...
(defn mydestructure [movie]
  (let [id (key movie)
        v (val movie)]
    (println (get v :title), (get v :genres))))
...
(defn -main []
  ...
  (println "5:10 premieres evals - 6:film avg min&max - 7: moyenne des moyennes - 8: user avg mi&max")
  ...
  ;;question2
  (= n 6) (let [avg (movie-avg-rating)] (println "films avg min&max" (avg-minmax avg)))
