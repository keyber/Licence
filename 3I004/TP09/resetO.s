...
reset:
    ...
    la      $26,	_isr_ioc
    sw      $26,	0($27)  /* interrupt_vector[0] <= _isr_ioc */
    la      $26,	_isr_dma
    ...
    /* initializes SR for tasks 1,2,3 (set IM, UM, EXL, IE) */
    li      $26,    0x0000FF13
	...
	/*dans seg_data_base se trouve l'adresse de main0*/
    mtc0    $26,    $14                 /* EPC for task 0 */
	/*eret retourne a l'adresse contenue dans EPC qui est l'adresse du main0*/
    li      $26,    0x0000FF13          /* set IM, UM, EXL and IE */
	/*mode kernel avec les interruptions désactivé car EXL = 1*/
    mtc0    $26,    $12                 /* SR for task 0 */

    /* jumps to address contained in EPC (in user mode) */
to_user:
    eret /* SOCCUPE DE REMETTRE LES INTERRUPTIONS */
