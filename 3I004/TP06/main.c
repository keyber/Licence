...
#define Idct(i,j)          Idct[8*i+j]
    ...
    int Yc[BLOCK_HEIGHT], r;

    tty_printf("IDCT thread is alive!\n");
    ...
    for ( block=0; block<NBLOCKS; ++block ) {
        ...
        Idct = out;
        i = (i+1) % 6;
        tty_printf("IDCT processing block %d/%d\n",
                block, NBLOCKS);
        ...
        tty_printf("IDCT: %d blocks in %d cycles\n", block, end-begin);
    } // end for block = fin image

    end = proctime();
    tty_printf("+++ IDCT total time = %d cycles\n", end );
