...
/* TME8 */
static region *creer_region( unsigned char *buf, int ix, int iy, int x, int y, int dimx) {
  ...
  for (j = 0; j < r->y; ++j) {
	  for (i = 0; i < r->x; ++i) {
		  float v = buf[i + r->ix - 1 + dimx * (j + r->iy -1)];
		  r->var += (v - r->mu)*(v - r->mu);
	  }
  }
  r->var /= r->n;
...
static void split_rec( unsigned char *buf, int dimx, GNode *noeud, float t, int xmin, int ymin) {
  region *r = noeud -> data;
  region *sreg;
